...
export function BalanceForm({ cash, card }: { cash: number; card: number }) {
  ...
  return <form className="grid gap-4" onSubmit={(event) => {
    ...
    startTransition(async () => {
      const result = await saveCurrentBalances({ cashBalance, cardBalance });
      if ("error" in result) toast.error(result.error); else toast.success("Joriy balans saqlandi");
    });
  }}>
