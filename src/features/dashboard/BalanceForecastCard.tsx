...
export function BalanceForecastCard({ forecast }: { forecast: BalanceForecast }) {
  return <Card className="overflow-hidden border-primary/15 bg-gradient-to-br from-card to-primary/[0.04]">
    ...
    <CardContent className="grid gap-4 sm:grid-cols-3">
      ...
      <p className="text-xs text-muted-foreground sm:col-span-3">Hisob {forecast.balanceDate ? `${forecast.balanceDate} sanasidan boshlab ` : ""}chiqim kiritilgan eng so‘nggi {forecast.reportDayCount} kun asosida olinadi (ko‘pi bilan 30 kun). Ta’mir, qurilma, sayohat va sovg‘a kabi bir martalik sarflar o‘rtachaga kiritilmaydi.</p>
    </CardContent>
  </Card>;
}
