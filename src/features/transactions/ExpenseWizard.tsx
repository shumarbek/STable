    ...
    <section className="mx-auto min-w-0 w-full max-w-2xl">
      <div className="overflow-hidden rounded-3xl border bg-card shadow-sm">
        <header className="relative flex min-h-20 items-center border-b px-4 sm:px-6">
          ...
          <div className="pointer-events-none absolute inset-x-12 text-center sm:inset-x-16"><p className="truncate font-heading text-lg font-bold sm:text-2xl">{step === "category" ? "Chiqim qo‘shish" : selectedType?.name ?? selectedCategory?.name}</p>{step !== "category" && <p className="mt-0.5 truncate text-xs text-muted-foreground sm:text-sm">{step === "type" ? "Chiqim turini tanlang" : "Bandlarga summa va to‘lov usulini kiriting"}</p>}</div>
        </header>
        <div className="p-4 sm:p-6">
          ...
          {step === "details" && selectedType && <div>
            ...
            <div className="grid gap-2.5">{items.map((item, index) => <div key={item.id} className={cn("grid min-w-0 grid-cols-[2.5rem_minmax(0,1fr)_6.75rem] items-center gap-2 rounded-2xl border p-2.5 transition-colors sm:grid-cols-[auto_1fr_11rem] sm:gap-3 sm:p-3", (item.isZeroConsumption || Number(item.amount) > 0) && "border-primary/40 bg-primary/[0.025]")}><IconBubble icon={item.icon} index={index} /><div className="min-w-0"><p className="truncate text-sm font-semibold sm:text-base">{item.name}</p><div className="mt-1.5 flex items-center gap-1 sm:mt-2 sm:gap-2"><Banknote className={cn("size-3.5 shrink-0 sm:size-4", item.paymentMethod === "cash" ? "text-primary" : "text-muted-foreground")} /><Switch checked={item.paymentMethod === "card"} onCheckedChange={(checked) => setItems((current) => current.map((entry) => entry.id === item.id ? { ...entry, paymentMethod: checked ? "card" : "cash" } : entry))} aria-label={`${item.name}: chapda naqd, o‘ngda karta`} /><CreditCard className={cn("size-3.5 shrink-0 sm:size-4", item.paymentMethod === "card" ? "text-primary" : "text-muted-foreground")} /><span className="hidden text-xs text-muted-foreground sm:inline">{item.paymentMethod === "cash" ? "Naqd" : "Karta"}</span></div></div><div className="flex min-w-0 items-center gap-1 sm:gap-2"><Input value={item.amount} type="number" min={0} step={100} inputMode="numeric" placeholder="0" disabled={item.isZeroConsumption} onChange={(event) => setItems((current) => current.map((entry) => entry.id === item.id ? { ...entry, amount: event.target.value } : entry))} className="h-10 min-w-0 px-2 text-right text-sm font-semibold tabular-nums sm:h-11 sm:text-base" aria-label={`${item.name} summasi`} /><span className="shrink-0 text-[11px] text-muted-foreground sm:text-sm">so‘m</span></div></div>)}</div>
            <div className="mt-5 rounded-2xl border bg-card p-3 shadow-sm"><div className="mb-3 flex items-center justify-between px-1"><span className="text-sm text-muted-foreground">{activeItems.length} ta band</span><span className="font-heading text-lg font-bold tabular-nums">{formatMoney(total)}</span></div><Button className="h-12 w-full rounded-xl text-base" disabled={isPending || activeItems.length === 0} onClick={save}>{isPending && <Loader2 className="size-4 animate-spin" />}Saqlash</Button></div>
          </div>}
        </div>
      </div>
    </section>
